...
const checkProfileJwt = async (req: NextRequest): Promise<checkProfileResponseType> => {
  const lastCheck: string | undefined = req.cookies.get('jwtLastCheck')?.value;

  const sessionData = req.cookies.get('auth-session');
...
//     } else if (
//       req.nextUrl.pathname === '/' &&
//       userJwtValid.authenticationResult === 'unauthorized'
//     ) {
...
export async function middleware(req: NextRequest, res: NextResponse) {
  const protectedRoutes = ['/home', '/notifications', '/bookmarks', '/user'];
    ...
    const userJwtValid = await checkProfileJwt(req);
    const sessionData = req.cookies.get('auth-session');
    const sessionDataDecrypted = await getSession()

    console.log("middleware session", sessionData)
    console.log("decrypted from cookies headers", sessionDataDecrypted)
    ...
    if (isAuthPage && userJwtValid.authenticationResult === 'ok') {
      let res = NextResponse.redirect(new URL('/home', req.url));
      res.cookies.set('auth-session', sessionData!.value);
      return res;
    }
    ...
    // if (isAuthPage && userJwtValid.authenticationResult === 'ok') {
    //   let res = NextResponse.redirect(new URL('/home', req.url));
    //   res.cookies.set('auth-session', userJwtValid.jwtLastCheck);
    //   return res;
    // } else if (!isAuthPage && userJwtValid.authenticationResult === 'unauthorized') {
    //   for (let i = 0; i < protectedRoutes.length; i++) {
    //     const route = protectedRoutes[i];
